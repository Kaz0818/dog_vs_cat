...
class Trainer:
    ...
    def __init__(
        ...
        self.device = (
        device
        or torch.device("mps") if torch.backends.mps.is_available()
        ...
        self.writer = SummaryWriter(log_dir=str(self.log_dir)) if self.use_tensorboard else None
    ...
    def _log_scalars(self, epoch, train_loss, val_loss, train_acc, val_acc):
        if self.writer:
    ...
    def _maybe_step_scheduler(self, val_loss=None):
        if not self.scheduler:
            return
        # ReduceLROnPlateau 対応
        try:
    ...
    def _should_early_stop(self, val_loss, best_val_loss, epochs_no_improve):
        improved = val_loss < best_val_loss
        if improved:
            return False, 0  # 早期終了リセット
        if self.early_stopping_patience is None:
            return False, epochs_no_improve + 1
        return epochs_no_improve + 1 >= self.early_stopping_patience, epochs_no_improve + 1
    ...
    def train(self) -> Dict[str, List[float]]:
        ...
        try:
            for epoch in range(self.num_epochs):
                ...
                # TensorBoardへの記録
                self._log_scalars(epoch, train_loss, val_loss, train_acc, val_acc)
                ...
                # 早期終了判定
                should_stop, epochs_no_improve = self._should_early_stop(val_loss, best_val_loss, epochs_no_improve)
                if should_stop:
